  ...
  return (
    <div className="min-h-screen bg-surface">
      ...
      <main className="pb-20 lg:pb-0">
        <section className="border-b bg-background"><div className="mx-auto grid max-w-7xl gap-8 px-4 py-12 sm:px-8 lg:grid-cols-[1fr_420px] lg:items-end lg:py-16"><div><h1 className="max-w-3xl font-display text-4xl font-extrabold leading-tight sm:text-5xl">Pilihan Tepat,<br/><span className="text-primary">Untuk Kenikmatanmu.</span></h1><p className="mt-4 max-w-xl text-base leading-7 text-muted-foreground">Pilih menu favoritmu, masukkan nama, lalu bayar di kasir. Kami akan menyiapkannya selagi hangat.</p></div><div className="relative"><Search className="absolute left-4 top-1/2 size-5 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Cari makanan atau minuman..." className="h-14 rounded-2xl bg-surface pl-12 text-base shadow-sm" /></div></div></section>
      ...
      {count > 0 && (
        <div className="fixed inset-x-0 bottom-0 z-20 px-4 lg:hidden">
          <Sheet>
            ...
            <CartSheet items={items} total={total} name={name} setName={setName} update={update} checkout={checkout} submitting={submitting} />
          </Sheet>
        </div>
      )}
    </div>
